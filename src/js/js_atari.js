(function () {
  ...
  function start_emu(cart) {
    ...
    cartridge_region = Cartridge.GetRegion();
    cartridge_left_switch = Cartridge.GetLeftSwitch();
    cartridge_right_switch = Cartridge.GetRightSwitch();

    // Notify other components that the cartridge has been loaded
    ...
    var prosystem_frequency = ProSystem.GetFrequency();
    ...
    var f = function () {
      if (ProSystem.IsActive() && !ProSystem.IsPaused()) {

        js_atari_update_keys(atari_keyboard_data);
        ProSystem.ExecuteFrame(atari_keyboard_data);
  ...
  function js_atari_start_emulation(cart) {
    ...
    $("#logo").fadeOut("slow");
    setTimeout(function () { screen_noise = false; start_emu(cart); }, 200);
  }
  ...
  function js_atari_init() {
    // Audio
    js_atari_init_audio();
    Sound.SetStoreSoundCallback(js_storeSound);
  ...
  */
  ...
  function js_atari_init_audio() {
    if (!atari_audio_ctx && (window.AudioContext || window.webkitAudioContext)) {
      ...
      if (atari_audio_ctx.sampleRate) {
        sampleRate = atari_audio_ctx.sampleRate;
      }
  ...
  js7800.init = js_atari_init;
  js7800.startEmulation = js_atari_start_emulation;
  js7800.keyEvent = js_atari_key_event;
})();
